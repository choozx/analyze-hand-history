...
- **🎯 문제 풀기** — 내가 자주 실수하는 스팟에서 문제를 출제하는 연습 모드.
  **실제 내 핸드**를 히어로의 결정 직전에서 잘라 보여주고(그 뒤 액션·다음 스트리트·쇼다운·결과는
  ...
  - **📐 오픈 레인지 모드** (같은 탭 안의 두 번째 모드) — "앞이 전부 폴드했다, 이 핸드 여나?"만
    묻는 속사 드릴. **출제도 채점도 전부 로컬 — AI 호출 0회**, 정답이 차트에 있어 즉시 채점됨.
    - 포지션(UTG/MP/CO/BTN/SB/헤즈업) × 스택(<15/15–25/25–40/40bb+) 별 RFI 차트 24장.
      **<15bb는 레이즈가 아니라 푸시(올인) 레인지**로 묻는다
    - 차트는 **공개 MTT 참고 차트의 근사**(솔버 실시간 출력이 아님). 그래서 경계 핸드는
      `mix`로 따로 두고 **오픈이든 폴드든 [무난]** — 확실한 구간만 [좋음]/[실수]
    - **내 실전 기록이 차트와 어긋난 조합이 더 자주 출제됨**(폴드 투 히어로 상황의 실제 RFI 기록 기준.
      `--rebuild` 안 된 구 DB에서는 균등 무작위로 자동 폴백)
    - 채점 후 **13×13 차트 그리드**를 열어 내 실제 오픈 비율을 겹쳐 볼 수 있음(어긋난 칸은 빨간 테두리)
    - 키보드: `O` 오픈 · `F` 폴드 · `Enter` 다음 문제
...
- **연습 트레이너 고도화** (기본형은 🎯 문제 풀기로 구현 완료) — 남은 것:
  - **ICM 반영 차트** — 오픈(RFI) 차트 판정은 📐 오픈 레인지 모드로 구현됨(`ranges.py`).
    남은 것은 버블/파이널테이블 ICM 보정과 3벳·디펜스 차트(현재는 오픈만).
...
| `quiz.py` | 🎯 문제 풀기(핸드 리뷰) — 약점 스팟 탐지(휴리스틱·통계 이탈)·문제 출제·채점 캐시/성적 |
| `ranges.py` | 📐 오픈 레인지 드릴 — 포지션×스택별 RFI 차트·출제·**로컬 채점**(AI 호출 없음) |
